...
object RemovingStarsFromAString {
    ...
    fun removeStars(input: String): String {
        ...
        val charsStack: ArrayDeque<Char> = ArrayDeque()

        for (current in inputArray) {
            if (current == '*' && !charsStack.isEmpty()) {
                charsStack.removeFirst()
            } else {
                charsStack.addFirst(current)
            }
        }

        val sb = StringBuilder()
        while (!charsStack.isEmpty()) {
            sb.append(charsStack.removeFirst())
        }
